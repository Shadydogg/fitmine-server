// /lib/refreshGoogleToken.js — v2.1.0
...
async function refreshGoogleToken(telegram_id) {
  ...
  try {
    ...
    console.log(`🔁 Новый access_token получен, истекает через ${expires_in}s → ${expire_at}`);
    ...
    if (updateError) {
      ...
      return { error: 'Failed to update token in Supabase' };
    }

    console.log(`✅ [refreshGoogleToken] access_token обновлён для ${telegram_id}`);
    ...
    console.error('❌ Ошибка при обновлении Google токена:', err.response?.data || err.message);
